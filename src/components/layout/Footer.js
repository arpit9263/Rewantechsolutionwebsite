...
export default function Footer() {
  return (
    <footer style={{ background:'var(--bg2)', borderTop:'1px solid var(--bdr)', position:'relative', overflow:'hidden' }}>
      <div className="noise"/>
      {/* CTA Band */}
      <div style={{ background:'linear-gradient(135deg,rgba(37,99,235,0.08) 0%,rgba(6,182,212,0.05) 100%)', borderBottom:'1px solid var(--bdr)', padding:'clamp(52px,7vw,80px) clamp(20px,5vw,80px)', position:'relative', zIndex:1 }}>
        ...
        <div className="ctr" style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:40, position:'relative', zIndex:1 }}>
          <div>
            ...
            <h2 className="d2" style={{ fontSize:'clamp(26px,4vw,52px)', marginBottom:14 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <a href="https://wa.me/917992127047?text=Hi%2C+I+want+to+discuss+a+project" target="_blank" className="btn btn-wa btn-lg" style={{ justifyContent:'center' }}>
      ...
      {/* Main footer */}
      <div style={{ padding:'clamp(52px,6vw,72px) clamp(20px,5vw,80px) clamp(28px,4vw,40px)', position:'relative', zIndex:1 }}>
        <div className="ctr">
          <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr 1.2fr', gap:'clamp(32px,4vw,60px)', marginBottom:56 }}>
            {/* Brand */}
            <div>
              <Link href="/" style={{ display:'inline-flex', alignItems:'center', gap:10, marginBottom:20 }}>
                <div style={{ width:32, height:32, borderRadius:9, background:'linear-gradient(135deg,var(--b),var(--c))', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 0 16px rgba(37,99,235,0.35)' }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M4 8h10a4 4 0 0 1 0 8H4" stroke="white" strokeWidth="2.2" strokeLinecap="round"/><circle cx="19" cy="16" r="1.8" fill="rgba(255,255,255,0.7)"/></svg>
                </div>
                <span style={{ fontFamily:'var(--f-d)', fontWeight:800, fontSize:15, letterSpacing:'-0.05em', color:'var(--t1)' }}>rewan<span style={{ color:'var(--bl)' }}>tech</span></span>
              </Link>
              <p className="body-sm" style={{ maxWidth:280, marginBottom:24, lineHeight:1.8 }}>Premium IT engineering partner. We design, build, and scale digital products that drive real business outcomes.</p>
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {[
                  { icon:<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>, v:'rewantechsolutions@gmail.com', h:'mailto:rewantechsolutions@gmail.com' },
                  { icon:<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.38 2 2 0 0 1 3.58 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 8.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>, v:'+91 7992127047', h:'tel:+917992127047' },
                ].map(c=>(
                  <a key={c.v} href={c.h} style={{ display:'inline-flex', alignItems:'center', gap:8, fontFamily:'var(--f-d)', fontSize:12, color:'var(--t3)', transition:'color 0.2s' }}
                    onMouseEnter={e=>e.currentTarget.style.color='var(--bl)'}
                    onMouseLeave={e=>e.currentTarget.style.color='var(--t3)'}>
                    <span style={{ color:'var(--t3)' }}>{c.icon}</span>{c.v}
                  </a>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <a href="https://wa.me/917992127047" target="_blank" style={{ display:'flex', alignItems:'center', gap:10, background:'rgba(37,211,102,0.07)', border:'1px solid rgba(37,211,102,0.15)', borderRadius:'var(--rl)', padding:'14px 16px', marginBottom:10, transition:'all 0.25s' }}
                ...
                <div>
                  ...
                  <div style={{ fontFamily:'var(--f-d)', fontSize:10, color:'var(--t3)' }}>+91 8545098444</div>
                </div>
              </a>
              <Link href="/contact" style={{ display:'flex', alignItems:'center', gap:10, background:'var(--bg-b)', border:'1px solid var(--ln-b)', borderRadius:'var(--rl)', padding:'14px 16px', transition:'all 0.25s' }}
          ...
          {/* Bottom bar */}
          <div style={{ borderTop:'1px solid var(--bdr)', paddingTop:24, display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:16 }}>
            ...
            <div style={{ display:'flex', gap:20 }}>
              {['Privacy Policy','Terms of Service','Sitemap'].map(t=>(
                <span key={t} style={{ fontFamily:'var(--f-d)', fontSize:11, color:'var(--t4)', transition:'color 0.2s' }}
                  onMouseEnter={e=>e.currentTarget.style.color='var(--bl)'}
